...
**Run**:
A single playthrough, from the Champion's starting stats and the Ladder's baseline state until the Champion dies in a Fight or clears every rung. Pressing Commit to Fight only starts the Run: after that Fights chain automatically, with a cooldown after each one, until the Champion loses or clears the final rung. A loss ends the Run and, after the cooldown, triggers a Restart. Clearing the final rung leaves the Run in a victorious state that only the player's Prestige moves past.
...
**Restart**:
What happens automatically after a Fight loss: once a short cooldown elapses, the Run is wiped back to the start of the Ladder (Champion back to base stats with no inherited Traits, Enemies back to their authored baseline plus permanent Enemy Upgrades). A Restart grants nothing — no prestige token, no Ladder Level. The player then presses Commit to Fight to begin the next Run. Gold, Experience, and Enemy Upgrades survive because they live in the Progression store.
_Avoid_: Reset (an earlier term that conflated Restart and Prestige — do not use it for either)

**Prestige**:
The player-clicked action available only after the Champion has cleared the final rung. It grants exactly 1 prestige token and permanently increments the global Ladder Level, then restarts the Run. Never available after a defeat. The unique/fun twist on this classic prestige shape is still deferred.
_Avoid_: Reset

**Experience**:
A permanent meta-progression currency held in a separate Progression store, independent of Run state. Rather than a flat per-Fight amount, each Enemy carries its own `experienceValue`, seeded at authoring time; this grows exactly like a Trait does when Enemies fight each other in the autonomous ecosystem (winner takes the loser's current value, loser resets to zero). Defeating an Enemy in a Champion Fight banks that Enemy's current `experienceValue` — passed through a swappable formula keyed to how far up the Ladder it was reached — permanently into the Progression store. A Fight *loss* grants no new Experience; Experience already banked from earlier wins in that Run is unaffected, since it lives outside the Run store and survives the Run ending. This is a deliberate reversal of an earlier flat "granted win or loss" design: the player is still guaranteed some Experience-earning path per Run because early/weaker Enemies stay reliably beatable, but the loss itself now pays nothing further.
...
- **Ladder reordering**: after a Prestige, the player may be able to choose or rearrange which Enemy occupies which rung, as a strategic layer on top of Trait placement. Flagged as a near-term priority — sooner than mutation, considered must-have core gameplay — but not yet designed; needs its own grilling pass once there are enough Enemies and Traits for placement to matter. A UI-layout prototype session (see Ladder view layout, below) reconfirmed this gating from the layout side too: none of the three explored layouts had anywhere sensible to put a placement interaction, because the autonomous Enemy-vs-Enemy ecosystem (see Enemy) isn't simulated in code yet — there's nothing concrete yet to place Enemies onto or design a control for.
